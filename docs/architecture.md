...
TF-IDF is used before FAISS, Chroma, or another vector database on purpose. The current milestone needs a lightweight lexical baseline that works immediately with SQLite chunks, has no embedding model dependency, requires no background index server, and is easy to test with deterministic expectations. This keeps the local-first MVP understandable: a query is matched against stored chunk terms, the score is reproducible, and tests can assert ranking behavior without relying on model downloads or approximate nearest-neighbor indexes.

FAISS, Chroma, or another vector store can be added later when semantic retrieval is needed across larger document sets. That upgrade should be driven by measured gaps in the TF-IDF baseline, such as synonym-heavy questions, cross-paper semantic comparison, or poor recall when users ask conceptual questions that do not share terms with the source text. Until then, TF-IDF is the right foundation because it is simpler to debug, cheaper to run, and honest about what it can and cannot retrieve.
